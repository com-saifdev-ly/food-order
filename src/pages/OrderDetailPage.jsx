  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        // Get specific order with retry logic for newly created orders
        let orderData;
        let retries = 0;
        const maxRetries = 3;
        ...
        while (retries < maxRetries) {
          try {
            orderData = await getCustomerOrder(session.user.id, orderId);
            if (orderData) break;
          } catch (error) {
            retries++;
        ...
        if (orderData.customer_id !== session.user.id) {
          setMessage(copy.noPermissionViewOrder);
          setLoading(false);
          return;
        }
    ...
    return (
      <PageShell language={language}>
        <section className="Hero-card">
          ...
          <a className="Primary-btn" href={getLocalizedPath('/orders', language)} style={{ marginTop: '16px' }}>
            {copy.backToOrders}
          </a>
        </section>
      </PageShell>
              ...
              <p style={{
                ...
                <span style={{
                  ...
                  ${order.order_items
                    ...
                    .reduce((sum, item) => {
                      const priceValue = typeof item.price === 'string' && item.price.startsWith('$')
                ...
                >
                  ...
                  {expandedItems[item.id] ? (
                    ...
                    <div style={{ padding: '8px', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                      <span>{copy.quantityLabelShort}: {item.quantity}</span>
                      ...
                      {item.price && <span> • {copy.priceLabel}: {typeof item.price === 'string' && item.price.startsWith('$') ? item.price : `$${item.price}`}</span>}
                    </div>
                  )}
                  <div className="Order-item-row" style={{ marginTop: '12px', display: 'flex', justifyContent: 'flex-start' }}>
                    {item.status === 'pending' && (
                      ...
                      >
                        {deletingItem === item.id ? copy.deletingButton : copy.deleteButton}
                      </button>
                    )}
                  </div>
                </div>
